...
expectf :: proc(t: ^testing.T, ok: bool, format: string, args: ..any, loc := #caller_location) -> bool {
    if !ok {
...
test_reading1 :: proc(t: ^testing.T) {
    ...
    constant_value := cr.cp_find(
        classfile, cr.ConstantIntegerInfo,
...
@private
test_method :: proc(t: ^testing.T, classfile: cr.ClassFile, name, descriptor, declaring_class: string) {
...
@private
test_field :: proc(t: ^testing.T, classfile: cr.ClassFile, name, descriptor, declaring_class: string) {
    ...
    // now validate the constant pool
    cp_field := cr.cp_find(
        classfile, cr.ConstantFieldRefInfo,
